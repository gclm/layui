    ...
    var debug = $('#L_layerDebug'),
        popDebug = function() {
            ...
            layer.open({
                ...
                maxWidth: '100%',
                offset: [
                    (othis.offset().top - top + 50) + 'px', (othis.offset().left - 300) + 'px'
                ],
...
return false;
    ...
    (function() {
        var lis = gather.api.find('li'),
            slecked = 'layer-api-slecked';
    ...
    layer.ready(function() {
        ...
        if (0 && !notAliyun) {
            layer.alert('', {
                ...
                content: ['<div style="text-align: center">'
                    ...
                    , '<p>三年仅 <span style="color: #FF5722;">203元</span> 购买阿里云服务器</p>', '<p style="color: #999;">新春之计 万象更新</p>', '</div>'
                ].join(''),
                btn: ['赶紧去围观', '等会看'],
                btnAlign: 'c',
                success: function(layero, index) {
        ...
        if (elemNavTips[0] && !notNavTips && 0) {
            var tipsIndex = layer.tips(
                ['<a href="https://fly.layui.com/jump/txyhot/" target="_blank" id="layer-spm-event-parter-tips" style="display: block; line-height: 30px; padding: 10px; text-align: center; font-size: 14px; background-color: #BE9241; background-image1: linear-gradient(to right,#79693F,#79693F,#4E4632,#302E2A); color: #fff;">', '<p>腾讯云产品限时秒杀</p>', '<p style="">爆款服务器超低价</p>', '</a>'].join(''), '#layer-spm-event-parter', {
                    tips: [3, '#BE9241'],
                    maxWidth: 320,
                    time: 0,
                    anim: 2,
        ...
        if (!notLayuiAdmin && false) {
            layer.open({
                ...
                moveType: 1 //拖拽模式，0或者1
                    ,
                resize: false,
                content: ['<div style="padding: 15px; text-align: center; background-color: #e2e2e2;">', '<a href="https://www.layui.com/admin/std/dist/views/" target="_blank"><img src="//cdn.layui.com/upload/2018_5/168_1527691799254_76462.jpg" alt="layuiAdmin" style="width: 100%; height:149.78px;"></a>', '</div>'].join(''),
                success: function(layero, index) {
                    var btn = layero.find('.layui-layer-btn');
                    btn.find('.layui-layer-btn0').attr({
